...
int main(int argc, char *argv[]) {
	...
	// allocate renderTiles
	mainRenderer.renderTiles = (struct renderTile*)calloc(1, sizeof(struct renderTile));
...
void getInput() {
	...
	while (userInput != 'q')
	{
		...
		{
		case 'w':
			addCamTransform(mainRenderer.scene->camera, newTransformRotateX(-30));
			transformCameraIntoView(mainRenderer.scene->camera);
			quantizeImage();
			computeFocalLength(&mainRenderer);
			mainRenderer.isRendering = true;
			return;
		case 's':
			...
			return;
		case 'a':
			addCamTransform(mainRenderer.scene->camera, newTransformRotateZ(-30));
			transformCameraIntoView(mainRenderer.scene->camera);
			quantizeImage();
			computeFocalLength(&mainRenderer);
			mainRenderer.isRendering = true;
